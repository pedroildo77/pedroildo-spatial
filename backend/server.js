...
async function publicUrl(bucket, filePath) {
...
app.post('/api/model', uploadMemory.single('model'), async (req, res) => {
  try {
    ...
    const public_url = await publicUrl(MODELS_BUCKET, storagePath);
...
app.post('/api/worldmap', uploadMemory.single('worldmap'), async (req, res) => {
  try {
    ...
    const url = await publicUrl(WORLDMAPS_BUCKET, storagePath);
